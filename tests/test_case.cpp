...
BOOST_FIXTURE_TEST_CASE(assert_works, yatf_fixture) {
    ...
    REQUIRE_EQ(1, 1);
    BOOST_CHECK_EQUAL(get_failed(), 0);
    BOOST_CHECK_EQUAL(get_assertions(), 1);
    REQUIRE_EQ(1, 0);
    BOOST_CHECK_EQUAL(get_failed(), 1);
    BOOST_CHECK_EQUAL(get_assertions(), 2);
    REQUIRE(true);
    BOOST_CHECK_EQUAL(get_failed(), 1);
    BOOST_CHECK_EQUAL(get_assertions(), 3);
    REQUIRE(false);
    BOOST_CHECK_EQUAL(get_failed(), 2);
    BOOST_CHECK_EQUAL(get_assertions(), 4);
    REQUIRE_FALSE(true);
    BOOST_CHECK_EQUAL(get_failed(), 3);
    BOOST_CHECK_EQUAL(get_assertions(), 5);
    REQUIRE_FALSE(false);
    BOOST_CHECK_EQUAL(get_failed(), 3);
    BOOST_CHECK_EQUAL(get_assertions(), 6);
    REQUIRE_EQ("hello", "world");
    BOOST_CHECK_EQUAL(get_failed(), 4);
    BOOST_CHECK_EQUAL(get_assertions(), 7);
    REQUIRE_EQ("hello", "hello");
    BOOST_CHECK_EQUAL(get_failed(), 4);
    BOOST_CHECK_EQUAL(get_assertions(), 8);
}

BOOST_FIXTURE_TEST_CASE(test_can_pass, yatf_fixture) {
    dummy_test_case tc{"suite_name", "test_name"};
    test_session::get().current_test_case(&tc);
    BOOST_CHECK_EQUAL(std::string{get_suite_name()}, "suite_name");
    BOOST_CHECK_EQUAL(std::string{get_test_name()}, "test_name");
    BOOST_CHECK_EQUAL(get_assertions(), 0);
    BOOST_CHECK_EQUAL(get_failed(), 0);
